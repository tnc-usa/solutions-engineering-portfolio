...
Low-code gets dismissed as toy software: fine for a form, not for anything that runs a business. I have spent the last few years proving the opposite, building and running thirteen production apps for cash operations on AppSheet, Google Apps Script, and Google Sheets. The platform is not the limit. The discipline around it is. Here is the discipline I put in place, because none of it comes in the box.

Version control for code that lives in a cloud editor

Apps Script runs in Google's editor, not in a repo. There is no git history, no diff, no branch. Change something in the cloud and your local copy is silently stale, or the reverse. On a thirteen-project estate that drift is a real production risk.
